...
## Project constraints (override generic landing advice)

UI-UX Pro Max picked **Vibrant & Block-based** + **Feature-Rich Showcase**. For this shop, apply tokens — not the SaaS landing pattern.

- **Brand first:** first viewport = shop name + one headline + one sentence + one CTA group + one full-bleed product image. No feature-card grid in the hero.
- **No cards in the hero.** Cards only wrap a real interaction (filters, cart line, checkout form).
